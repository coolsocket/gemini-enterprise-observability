...
1. **Chat prompt ↔ response 配对**。只有 `v1alpha` (REST) 聊天会产生配对的 `gen_ai.choice` 日志(共享 `trace_id`)。UI 走 `v1main` —— prompt 进 `user_activity`,但**不写 `choice`** → 网页发起的对话都是 `join_status='no_response'`。Conversations 页 "matched / prompt-only" filter 让这一点一目了然。

2. **Deep Research prompt + response 文本**。提交事件会记 (`AssistantService.AsyncAssist`,一次提交一行 —— 计数准) 但 prompt 文本和响应正文**不 emit 到 Cloud Logging**。要看内容去 GE 后台 Deep Research 任务列表。
...
14. **GE Console toggle 必须手工开**。每个 engine 去 GE Admin Console 打开 OpenTelemetry Instrumentation + Prompt & Response Logging (+ 可选 Feedback)。不开 `gen_ai.*` 日志根本不流,大多数 view 是空的。见 [`docs/GE_CONSOLE_SETUP.md`](./docs/GE_CONSOLE_SETUP.md)。
